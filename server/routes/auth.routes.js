...
// Validation Regex
const REG_NO_REGEX = /^7155\d{2}(1050|1120)\d{2}$/; // Allows EEE (1050) and ICE (1120)
...
router.post('/register', async (req, res) => {
    ...
    if (!REG_NO_REGEX.test(reg_no)) {
        return res.status(400).json({ error: 'Invalid Register Number format. Must be EEE (1050) or ICE (1120).' });
    }
    ...
    try {
        ...
        */
        ...
        // 4. Parse Roll Number for Classification
        let batch_year = null;
        let department = null;
        let class_name = null;

        // Roll No Logic: 7155|23|1050|02 (College|Batch|Dept|ID)
        // Check if starts with College Code
        if (reg_no.startsWith('7155') && reg_no.length >= 10) {
            const batchCode = reg_no.substring(4, 6); // Characters at index 4,5 (e.g. '23')
            const deptCode = reg_no.substring(6, 10); // Characters at index 6,7,8,9 (e.g. '1050')

            batch_year = batchCode;

            // Map Dept Code
            if (deptCode === '1050') {
                department = 'EEE';
            } else if (deptCode === '1120') {
                department = 'ICE';
            } else {
                department = 'Unknown';
            }

            class_name = `${batch_year}-${department}`;
        }

        // 5. Create New User (if not exists)
        const hashedPassword = await bcrypt.hash(password, 10);
        const newUser = await db.query(
            'INSERT INTO users (name, email, reg_no, password_hash, batch_year, department, class_name) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id, name, email, role, batch_year, department, class_name',
            [name, email, reg_no, hashedPassword, batch_year, department, class_name]
        );
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Backfill Classification if missing
        if (!user.batch_year || !user.department) {
            console.log(`Backfilling data for ${user.reg_no}`);
            const reg_no = user.reg_no;
            let batch_year = null;
            let department = null;
            let class_name = null;

            if (reg_no.startsWith('7155') && reg_no.length >= 10) {
                batch_year = reg_no.substring(4, 6);
                const deptCode = reg_no.substring(6, 10);

                if (deptCode === '1050') department = 'EEE';
                else if (deptCode === '1120') department = 'ICE';
                ...
                try {
                    await db.query(
                        'UPDATE users SET batch_year = $1, department = $2, class_name = $3 WHERE id = $4',
                        [batch_year, department, class_name, user.id]
                    );
        ...
        const token = jwt.sign(
            { id: user.id, email: user.email, role: user.role, name: user.name, reg_no: user.reg_no, batch_year: user.batch_year, department: user.department, class_name: user.class_name },
            JWT_SECRET,
            { expiresIn: '24h' }
        );

        res.json({ token, user: { id: user.id, name: user.name, email: user.email, role: user.role, reg_no: user.reg_no, batch_year: user.batch_year, department: user.department, class_name: user.class_name } });
